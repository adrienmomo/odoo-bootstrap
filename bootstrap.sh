...
# Create the odoo web data directory with proper permissions
mkdir -p "${SITENAME}-odoo-web-data"
mkdir -p "${SITENAME}-odoo-web-data/sessions"
# Set ownership to UID 101 (odoo user in the container)
sudo chown -R 101:101 "${SITENAME}-odoo-web-data" 2>/dev/null || echo "Warning: Could not set ownership. You may need to run: sudo chown -R 101:101 ${SITENAME}-odoo-web-data"
